...
public class ContaController {
    ...
    public ResponseEntity<RetornoDto> inserir(@Valid @RequestBody ContaDTO contaDTO){
        try {
    ...
    public ResponseEntity<ContaDTO> sacar(@Valid @RequestBody ContaDTO contaDTO, @RequestHeader (name="Authorization") String bearerToken){
        try {
            service.validarUsuario(obterDocumentoToken(bearerToken), contaDTO.getUuid());
    ...
    @PreAuthorize("hasAnyRole(T(tech.ada.banco.usuario.Role).ADMIN.name(),T(tech.ada.banco.usuario.Role).CLIENTE.name())")
    public ResponseEntity<RetornoDto> transferir(@Valid @RequestBody TransferenciaDTO contaDTO, @RequestHeader (name="Authorization") String bearerToken){
        try {
            service.validarUsuario(obterDocumentoToken(bearerToken),contaDTO.getContaOrigemUuid());
    ...
    private String obterDocumentoToken(String bearerToken){
        String token = bearerToken.substring(7);
        Usuario u = (Usuario) jwtService.getUserDetails(token);
        return u.getDocumento();
    }
